...
class AnthropicToOpenAIConverter:
    ...
    def convert_response(self, openai_response: Dict) -> Dict:
        ...
        # 🔥 关键修复：确保响应ID有效，防止空ID导致Claude Code重复请求
        original_id = openai_response.get('id', '')
        if not original_id or original_id == 'chat-':
            # 生成有效的Claude格式响应ID
            response_id = f"msg_{uuid.uuid4().hex[:12]}"
            logger.warning(f"OpenAI返回空响应ID '{original_id}'，生成新ID: {response_id}")
        else:
            # 使用原始ID，但转换为Claude格式
            response_id = f"msg_{original_id.replace('chat-', '')}" if original_id.startswith('chat-') else f"msg_{original_id}"
