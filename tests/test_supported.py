...
def test_supported_model():
    ...
    # 读取测试请求
    with open('test_mapped_model.json', 'r', encoding='utf-8') as f:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if content:
                print(f"- Content: {content[0].get('text', '❌')[:100]}...")
            
            # 保存响应到文件
            with open('supported_response.json', 'w', encoding='utf-8') as f:
                json.dump(response_data, f, indent=2, ensure_ascii=False)
            print(f"\n💾 响应已保存到 supported_response.json")
